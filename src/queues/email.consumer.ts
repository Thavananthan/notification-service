...
async function consumeAuthEmailMessages(channel: Channel): Promise<void> {
  try {
    ...
    const exchangeName = 'eapp-email-notification';
    const routingKey = 'auth-email';
    const queueName = 'auth-email-queue';
    await channel.assertExchange(exchangeName, 'direct');
    const eappQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false });
    await channel.bindQueue(eappQueue.queue, exchangeName, routingKey);
    channel.consume(eappQueue.queue, async (msg: ConsumeMessage | null) => {
...
async function consumeOrderEmailMessages(channel: Channel): Promise<void> {
  try {
    ...
    const exchangeName = 'eapp-order-notification';
