...
describe('Email Consumer', () => {
  ...
  describe('consumeAuthEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('eapp-email-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('auth-email-queue', 'eapp-email-notification', 'auth-email');
    });
  });
  ...
  describe('consumeOrderEmailMessages method', () => {
    it('should be called', async () => {
      ...
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('eapp-order-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('order-email-queue', 'eapp-order-notification', 'order-email');
    });
  });
});
